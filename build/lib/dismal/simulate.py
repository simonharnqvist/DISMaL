...
class MsprimeSimulation:
    ...
    def __init__(self, theta0, theta1, theta2, theta1_prime, theta2_prime,
        ...
        self.m_b2_to_b1 = self.number_to_proportion_migration_rate(
            m1_star, self.Ne_b)
        self.m_b1_to_b2 = self.number_to_proportion_migration_rate(
            m2_star, self.Ne)
        self.m_c2_to_c1 = self.number_to_proportion_migration_rate(
            m1_prime_star, self.Ne_c2)
        self.m_c1_to_c2 = self.number_to_proportion_migration_rate(
            m2_prime_star, self.Ne_c1)
    ...
    def simulate(self):
        ...
        demography.add_population_split(time=self.t0_gen_time, derived=[
                                        "b1", "b2"], ancestral="a")
        demography.add_population_split(
            time=self.t1_gen_time, derived=["c1"], ancestral="b1")
        demography.add_population_split(
            time=self.t1_gen_time, derived=["c2"], ancestral="b2")
        # NB: backwards in time in msprime
        demography.set_migration_rate("b2", "b1", self.m_b2_to_b1)
        ...
        for ts in [ts_state1, ts_state2, ts_state3]:
            ...
            for replicate_index, ts in enumerate(ts):
                ts_muts = msprime.sim_mutations(
                    ts, rate=self.mu_per_bp, discrete_genome=False)

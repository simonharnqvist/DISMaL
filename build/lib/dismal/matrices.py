...
class GeneratorMatrix():
    ...
    def __init__(self, matrix_type, m1=None, m2=None, m1_prime=None, m2_prime=None, a=None, b=None, c1=None, c2=None):
        ...
        if matrix_type == "Q1":
            assert c1 is not None and c1 > 0, f"invalid value {c1} for c1"
            assert c2 is not None and c2 > 0, f"invalid value {c2} for c2"
            ...
            assert b is not None and b > 0, f"invalid value {b} for b"
            ...
            assert a is not None and a > 0, f"invalid value {a} for a"
...
def p1(i,j, g, ginv, alpha, t):
    return np.sum([ginv[i,k] * g[k,j] * math.exp(-alpha[k]*t) for k in range(0,4)])

def p2(j,l, c, cinv, beta, t):
    return np.sum([cinv[j,k] * c[k,l] * math.exp(-beta[k]*t) for k in range(0,4)])